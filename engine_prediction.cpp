...
void c_engine_prediction::fix_viewmodel(int stage) 
{
	...
	if (anim_time != old_anim_time && cycle != old_cycle && cycle == 0.f && anim_time == HACKS->global_vars->curtime)
	{
		if (current_viewmodel->active_weapon == HACKS->weapon && current_viewmodel->sequence == viewmodel->sequence()
			&& current_viewmodel->animation_parity == viewmodel->animation_parity() && current_viewmodel->new_sequence_parity == viewmodel->new_sequence_parity()
			&& current_viewmodel->model_index == viewmodel->model_index() || current_viewmodel->looking_at_weapon == HACKS->local->looking_at_weapon())
		{
